...
final leaderboardProvider = FutureProvider<List<LeaderboardEntry>>(
  (ref) => ref.watch(leaderboardRepositoryProvider).globalFitnessXp(),
);
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text('Global Fitness XP', style: TextStyle(color: RunovaColors.textMuted)),
            const SizedBox(height: 18),
            Expanded(
              child: leaderboard.when(
                loading: () => const Center(child: CircularProgressIndicator()),
                error: (error, stack) => const Center(
                  child: Text('Sign in and check that the Runova API is running.'),
                ),
                data: (entries) => entries.isEmpty
                    ...
                    : ListView.separated(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return Card(
                            color: entry.isCurrentUser ? RunovaColors.elevatedSurface : null,
                            child: ListTile(
  ...
  Widget build(BuildContext context) {
    return const Center(
      child: Column(
        ...
        children: [
          Icon(Icons.emoji_events_outlined, size: 64, color: RunovaColors.warning),
          SizedBox(height: 18),
          Text('No rankings yet', style: TextStyle(fontWeight: FontWeight.w800)),
          SizedBox(height: 6),
