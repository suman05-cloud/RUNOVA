          ...
          children: [
            ...
            if (session.phase == RunPhase.idle ||
              ...
              FilledButton.icon(
                ...
                label: Text(
                  session.phase == RunPhase.completed ? 'NEW RUN' : 'START RUN',
                ),
              )
            else
              Row(
                children: [
                  Expanded(
                    child: FilledButton.tonalIcon(
                      ...
                      label: Text(
                        session.phase == RunPhase.paused ? 'RESUME' : 'PAUSE',
                      ),
                    ),
                  ),
                  ...
                  Expanded(
                    child: FilledButton.tonalIcon(
                      onPressed: session.busy ? null : controller.finish,
            ...
            if (session.result != null) ...[
              ...
              TextButton(
                onPressed: () =>
                    context.push('/history/${session.runId}?summary=true'),
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        ...
        Text(
          label,
          style: const TextStyle(color: RunovaColors.textMuted, fontSize: 11),
        ),
      ],
    );
  }
